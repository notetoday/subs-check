...
```bash
scp -P <PORT> socks5proxy root@<SERVER_IP>:/opt/subs-check/socks5proxy
```
...
```bash
# 节点池加载
curl --socks5-hostname 127.0.0.1:7890 https://<WORKER_HOST>/  -o /dev/null -w "%{http_code}\n"
# HTTP CONNECT
curl -x http://127.0.0.1:7890 https://<WORKER_HOST>/ -o /dev/null -w "%{http_code}\n"
```
